...
This project is an Adobe Edge Delivery Services (EDS) website.

Your primary objective is to implement features and fix issues while preserving the existing Franklin architecture, authoring experience, and performance.
...
4. Modify only the files required.
...
# Adobe Edge Delivery Rules

Always follow Franklin conventions.

Blocks must:

* remain reusable
* remain author-friendly
* support decoration
* gracefully handle missing content

Keep every block self-contained.
...
Do not create global block styles.
...
Never hardcode content that editors should manage.

Navigation is loaded from:

/nav

Footer is loaded from:

/footer

Do not duplicate navigation or footer logic inside pages.
...
* Explain why they need changing.
...
* If multiple approaches exist, recommend the Franklin/EDS approach.

Do not generate large code rewrites unless explicitly requested.

If requirements are ambiguous, ask clarifying questions before making architectural decisions.
...
1. User request
2. This AGENTS.md
3. Adobe Edge Delivery best practices
4. Existing project architecture
5. General programming conventions
